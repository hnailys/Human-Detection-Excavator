...
# Mengambil video
cap = cv2.VideoCapture("ReferenceVideos/video1.mp4")
# cap = cv2.VideoCapture("....../file_video.mp4")
...
def draw_boxes_and_distances(frame, boxes, confidences, label, distances):
    ...
    for i, box in enumerate(boxes):
        ...
        print(f"Box {i}: {box}, Label: {label}, Confidence: {conf}, Distance: {distance}")
...
frame_count = 0  # Inisialisasi penghitung frame
...
print(f"Total humans detected in 600 frames: {total_humans_detected}")
print(f"Total close detections (<3m) in 600 frames: {close_detections}")
